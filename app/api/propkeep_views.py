...
class PropkeepScanView(APIView):
    """PROPKEEP AUDIT — scan lease/policy text for compliance constraints."""
    permission_classes = [AllowAny]

    def post(self, request):
        from propkeep_brain import audit_lease, detect_state
        text = request.data.get('text', '') or request.data.get('lease', '')
        state = request.data.get('state') or detect_state(text)
        if not text or len(text) < 20:
            return Response({'error': 'lease text required (min 20 chars)'}, 400)
        return Response(audit_lease(text, state=state))
